...
erl_debugger_breakpoint_3(BIF_ALIST_3) {
    ...
    if (!code_hdr->line_breakpoints_supported) {
...
static Eterm
stack_frame_fun_info(Process *p, ErtsCodePtr pc, bool is_return_addr) {
    Eterm fun_info;
    FunctionInfo fi;
    ...
    if (fi.mfa == NULL) {
        ...
        Eterm *hp, mfa;
        int tup_arity = 2, mfa_arity = 3;

        hp = HAlloc(p, (tup_arity + 1) + (mfa_arity + 1));
        ...
        *hp++ = make_small(fi.mfa->arity);
        ...
        fun_info = make_tuple(hp);
        *hp++ = make_arityval(tup_arity);
        *hp++ = mfa;
        *hp++ = fi.loc == LINE_INVALID_LOCATION
                ? am_undefined
...
static Eterm
make_catch_tuple(Process *p, ErtsCodePtr catch_addr) {
    int tup_arity = 2;
    Eterm result, *hp;
    ...
    *hp++ = stack_frame_fun_info(p, catch_addr, false);
...
erl_debugger_stack_frames_2(BIF_ALIST_2)
{
    ...
    for(Eterm *sp = STACK_START(rp) - 1; rp->stop - 1 <= sp; sp--) {
        bool is_last_iter = (rp->stop - 1 == sp);
        int tup_arity;
        Eterm *hp, x;
        ...
        if (is_CP(x)) {
            bool is_return_addr = !is_last_iter;
            ...
            tup_arity = 3;
            hp = HAlloc(BIF_P, 2 /* cons */ + (tup_arity + 1) /* frame_info */);

            frame_info = make_tuple(hp);
            *hp++ = make_arityval(tup_arity);
            *hp++ = make_small(frame_no++);
            *hp++ = stack_frame_fun_info(BIF_P, code_ptr, is_return_addr);
            *hp++ = yregs;

    	    result = CONS(hp, frame_info, result);
