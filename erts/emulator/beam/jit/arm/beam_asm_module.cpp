...
void BeamGlobalAssembler::emit_i_line_breakpoint_trampoline_shared() {
    ...
    runtime_call<const Export* (*)(Process*, ErtsCodePtr, Uint, Eterm *, UWord *), erts_line_breakpoint_hit__prepare_call>();
    ...
    emit_enter_runtime<Update::eXRegs>();
    runtime_call<Uint (*)(Eterm *, UWord *), erts_line_breakpoint_hit__cleanup>();
    emit_leave_runtime<Update::eXRegs>();
...
enum erts_is_line_breakpoint BeamGlobalAssembler::is_line_breakpoint_trampoline(ErtsCodePtr addr) {
    ...
    if (pc == (const int32_t *)expected_target) return line_bp_type;
    ...
    auto mov_opcode = 0xD2800000 | super_tmp_reg; /* movz SUPER_TMP, #0, lsl #0 */
    ...
    for(int32_t hw = 0; hw < 4; hw++){
        ...
        instr = *++pc;
        mov_opcode = 0xF2800000 | super_tmp_reg; /* movk SUPER_TMP, #0, lsl #0 */
    };

    const int32_t expected_br_instr = 0xd61f0000 | (super_tmp_reg << 5); /* br SUPER_TMP */
    return (instr == expected_br_instr)
            ? line_bp_type
            : IS_NOT_LINE_BP;
}
