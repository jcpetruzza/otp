...
test_register_and_unregister_debugger(_Config) ->
    ...
    {ok, Ref1} = erl_debugger:register(Me),
    ...
    ok = erl_debugger:unregister(Me, Ref1),
    undefined = erl_debugger:whereis(),

    {ok, Ref2} = erl_debugger:register(Me),
    Me = erl_debugger:whereis(),
    BadRef = Ref2 + 1,
    {'EXIT', {badarg, _}} = catch erl_debugger:unregister(Me, BadRef),
    ok = erl_debugger:unregister(Me, Ref2),
    undefined = erl_debugger:whereis(),

    {ok, Ref3} = erl_debugger:register(AnotherProc),
    AnotherProc = erl_debugger:whereis(),
    erl_debugger:unregister(AnotherProc, Ref3),
    undefined = erl_debugger:whereis(),
    ok.
...
test_debugger_unregistered_when_dead(_Config) ->
    ...
    {ok, _Ref} = erl_debugger:register(Debugger),
