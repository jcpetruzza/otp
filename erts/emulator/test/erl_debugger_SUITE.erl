...
test_stack_frames_returns_frames(Config) ->
    ...
    ?assertMatch(
        [
            {4, {{Mod, base_level, 1}, _Line4 = 18}, [_Y0]},
            {3, {{Mod, one_level, 1}, _Line3 = 13}, []},
            {2, {{Mod, two_levels, 2}, _Line2 = 9}, []},
            {1, {{Mod, three_levels, 2}, _Line1 = 5}, []},
            {0, '<terminate process normally>', []}
        ],
        Actual
    ),
    ok.
...
test_stack_frames_returns_y_regs_controlled_by_size(Config) ->
    ...
    CheckVarsSize = fun(Size, ExpectedVars) ->
        case erl_debugger:stack_frames(P, Size) of
            [{1, {{Mod, args_as_yvars, 3}, Line}, YRegs} | _] ->
                ActualVars = #{
...
test_stack_frames_returns_catch(Config) ->
    ...
    ?assertMatch(
        [
            {2,
                {{call_stacks, call_with_catches_aux, 1}, 33},
                [
                    {value, 43},
                    {'catch', {{call_stacks, call_with_catches_aux, 1}, 33}}
                ]
            },
            {1,
                {{call_stacks, call_with_catches, 1}, 27},
                [
                    {value, 42},
                    {'catch', {{call_stacks, call_with_catches, 1}, 27}}
                ]
            },
            {0, '<terminate process normally>', []}
        ],
...
test_stack_frames_returns_breakpoint_frame(Config) ->
    ...
    ?assertMatch(
         [
            {3, {{erts_internal,breakpoint, 4}, _}, _},
            {2, '<breakpoint>', [_SavedX0 = {value, 42}, _SavedX1 = {value, 13}]},
            {1, {{call_stacks, three_levels, 2}, 5}, []},
            {0, '<terminate process normally>', []}
        ],
        Actual
    ),
...
test_stack_frames_works_with_hibernate(Config) ->
    ...
    ?assertMatch(
        [
            {1, {{erlang, hibernate, 3}, undefined}, []},
            {0, '<terminate process normally>',  []}
        ],
        Actual
    ),
...
test_peek_stack_frame_slot_works(Config) ->
    ...
    MaxYRegSize = lists:max([
        N
        || {_FrameId, _Fun, Slots} <- erl_debugger:stack_frames(P, 0),
        {too_large, N} <- Slots
    ]),
...
test_peek_xreg_works(Config) ->
    ...
    % While the process ir running, no results
