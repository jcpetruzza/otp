...
-export_type([stack_frame/0, stack_frame_fun/0, stack_frame_slot/0, xreg_val/0]).

-doc """
A stack-frame, including the value of each slot.
""".
-type stack_frame() ::
    {FrameNo :: non_neg_integer(), stack_frame_fun(), [stack_frame_slot()]}.

-doc """
What is running in each stack frame, including special VM frames.
""".
-type stack_frame_fun() ::
    {mfa(), Line :: pos_integer() | undefined}
    | '<terminate process>'
...
-type stack_frame_slot() ::
    {value, term()} | {too_large, Size :: pos_integer()} | {'catch', stack_frame_fun()}.
...
-spec peek_xreg(pid(), Reg, MaxSize) ->
    running | undefined | xreg_val() when
    Reg :: non_neg_integer(),
    MaxSize :: non_neg_integer().
peek_xreg(_, _, _) ->
    erlang:nif_error(undef).
