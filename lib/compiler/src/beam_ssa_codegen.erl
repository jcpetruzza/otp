...
get_original_name(#b_var{name=Name}, VarMap, Alias) ->
    Vs = [Name|get_original_name_1(Name, VarMap, Alias)],
    [V || V <- Vs, is_original_variable(V)].

get_original_name_1(Name, VarMap, Alias) ->
    case VarMap of
        #{Name := Var} ->
            [Var|get_original_name_1(Var, VarMap, Alias)];
        #{} ->
            []
    end ++ maps:get(Name, Alias, []).
